    ...
    try {
      ...
      for (let i = 0; i < nizaOfUsers.length; i++) {
          numberPosts[i] = {
            ...
            postovi: numberPosts[i],
          };
      };

      let postsUser = [];
      for(let i = 0; i < numberPosts.length; i++) {
        postsUser.push ("Korisnik: " + nizaOfUsers[i] + " | Broj na objavi " + numberPosts[i]);
      };
      console.log(numberPosts);
      console.log(postsUser);

      // zapishuvanje na postovi od user vo statistika.txt
      fs.writeFile('statistika.txt', JSON.stringify(postsUser, null, '\t'), (err) => {
